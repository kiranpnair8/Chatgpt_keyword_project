...
const apiKey = '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';  //Hugging Face API key
...
function sanitizeTitle(title) {
    return title.replace(/[^a-zA-Z0-9]/g, '_');  // To reeplace any non-alphanumeric characters with underscores
}
...
// Function to get a description of a painting
async function getDescription(title, painter) {
    ...
    try {
        ...
        return response.data[0].generated_text ? response.data[0].generated_text.trim() : 'No text generated';
    } catch (error) {
        console.error('Full error:', error.response ? error.response.data : error.message);
        throw new Error('Failed to generate description from Hugging Face');
    }
}
